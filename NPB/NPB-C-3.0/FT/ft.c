...
// 1 = initial conditions, 2 = single, 3 = fftintermidiates, 4 = evolve, 5 = inner specific
...
static void write3dArr(dcomplex x[NZ][NY][NX], const char *file)
{
	FILE *fp = fopen(file, "w");

	fprintf(fp, "%d %d %d-", NZ, NY, NX);
	...
	for (int k = 0; k < NZ; k++)
	{
		for (int j = 0; j < NY; j++)
		{
			for (int i = 0; i < NX; i++)
			{
				fprintf(fp, "%.*f %.*f\n", OP_DBL_Digs + 6, x[k][j][i].real, OP_DBL_Digs + 6, x[k][j][i].imag);
				// fprintf(fp, "[%d][%d][%d] %.*f %.*f\n", k, j, i, OP_DBL_Digs + 6, x[k][j][i].real, OP_DBL_Digs, x[k][j][i].imag);
			}
			// fprintf(fp, "\n");
		}
	}
...
static void write3dArrCorner(dcomplex x[NZ][NY][NX], int cornerSize, const char *file)
{
	FILE *fp = fopen(file, "w");
	// int cornerSize = 4;
	...
	for (int k = 0; k < cornerSize; k++)
	{
		for (int j = 0; j < cornerSize; j++)
		{
			for (int i = 0; i < cornerSize; i++)
			{
				fprintf(fp, "%.15e %.15e\n", x[k][j][i].real, x[k][j][i].imag);
			}
			fprintf(fp, "\n");
		}
	}
...
static void printi3dArrCorner(int x[NZ][NY][NX])
{
	int cornerSize = 4;
...
int main(int argc, char **argv)
{
	...
	if (WRITE_TO_FILE == 2)
	{
		write3dArr(u0, "/home/generic/luke/NPB/MPL-NPB/FT/ft_single_out_c.txt");
	}
	...
	for (iter = 1; iter <= niter; iter++)
	{
		...
		if (WRITE_TO_FILE == 4)
		{
			write3dArrCorner(u1, 4, "/home/generic/luke/NPB/NPB-C-3.0/out/ft_evolve_out_c.txt");
		}
		if (TIMERS_ENABLED == TRUE)
		{
			timer_stop(T_EVOLVE);
...
static void fft(int dir, dcomplex x1[NZ][NY][NX], dcomplex x2[NZ][NY][NX])
{
	...
	if (dir == 1)
	{
		...
		if (WRITE_TO_FILE == 3)
		{
			write3dArrCorner(x1, 4, "/home/generic/luke/NPB/NPB-C-3.0/out/ft_S1_out_c.txt");
		}
		cffts2(1, dims[1], x1, x1, y0, y1); /* x1 -> x1 */
		if (WRITE_TO_FILE == 3)
		{
			write3dArrCorner(x1, 4, "/home/generic/luke/NPB/NPB-C-3.0/out/ft_S2_out_c.txt");
		}
		cffts3(1, dims[2], x1, x2, y0, y1); /* x1 -> x2 */
		if (WRITE_TO_FILE == 3)
		{
			write3dArrCorner(x2, 4, "/home/generic/luke/NPB/NPB-C-3.0/out/ft_S3_out_c.txt");
		}
	}
	else
	{
		...
		if (WRITE_TO_FILE == 3)
		{
			write3dArrCorner(x1, 4, "/home/generic/luke/NPB/NPB-C-3.0/out/ft_S-3_out_c.txt");
		}

		cffts2(-1, dims[1], x1, x1, y0, y1); /* x1 -> x1 */
		if (WRITE_TO_FILE == 3)
		{
			write3dArrCorner(x1, 4, "/home/generic/luke/NPB/NPB-C-3.0/out/ft_S-2_out_c.txt");
		}

		cffts1(-1, dims[0], x1, x2, y0, y1); /* x1 -> x2 */
		if (WRITE_TO_FILE == 3)
		{
			write3dArrCorner(x1, 4, "/home/generic/luke/NPB/NPB-C-3.0/out/ft_S-1_out_c.txt");
		}
	}
}
