...
#if !defined(PART_OF_CG) && \
    !defined(STATIC_LINK_CG) && \
    !defined(DYNAMIC_LINK_CG)
#define PART_OF_CG
#endif
...
#elif defined(STATIC_LINK_CG)
#define MLLIB_PRIVATE(x)
#define MLLIB_PUBLIC(x) PUBLIC x
#elif defined(DYNAMIC_LINK_CG)
#define MLLIB_PRIVATE(x)
#define MLLIB_PUBLIC(x) EXTERNAL x
#else
#error Must specify linkage for cg
#define MLLIB_PRIVATE(x)
